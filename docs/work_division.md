...
This version reflects the **2026-04-17 pivot**: Jacob had already written a Java CLI in a separate repository, so the Python `cli/` and `web/` folders were archived to `legacy_python/`. The interchange between Alfred/Sky (this repo) and Jacob (Java repo) is now one CSV file per table, defined in [docs/csv_format.md](csv_format.md).
...
- **CSV interchange specification** — [docs/csv_format.md](csv_format.md). This is the contract that unblocks Sky's CSV generator and Jacob's Java `LOAD DATA INFILE`.
- Repository coordination: post-pivot cleanup ([legacy_python/](../legacy_python/)), [README.md](../README.md), and [AGENTS.md](../AGENTS.md).

Standalone: yes. Depends on nothing except the spec and the TA feedback. Does not call or read any code written by Sky or Jacob.
...
- [scripts/setup_db.py](../scripts/setup_db.py): orchestrator that runs the two generators above and leaves the nine CSV files ready in `data/csv/` for Jacob's Java loader to consume.
- Golden-sample CSVs committed under `data/csv_sample/` (≤ 10 rows per file) so Jacob can exercise his Java loader against FK-consistent data before the full dataset is generated.
...
Standalone: yes. Depends on Alfred's schema and CSV spec but not on any of Jacob's Java code.
...
- `LOAD DATA LOCAL INFILE` ingestion of the nine CSV files produced by Sky's generator, in the FK-safe order listed in [docs/csv_format.md](csv_format.md) — wrapped in `SET FOREIGN_KEY_CHECKS = 0 … 1` to avoid row-by-row FK validation overhead.
- [queries/easy.sql](../queries/easy.sql), [queries/medium.sql](../queries/medium.sql), [queries/hard.sql](../queries/hard.sql): nine canonical queries (E1–E3, M1–M3, H1–H3) that exercise the schema. SQL lives in this repo so Alfred and Sky can regression-test it; the Java CLI reads these files at runtime.
...
Standalone: yes. Depends on the CSV files from Sky but not on anyone's Python code beyond the shared [config.py](../config.py).
...
- **Python click CLI** (`legacy_python/cli/`) — replaced by Jacob's Java CLI.
